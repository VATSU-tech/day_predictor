# 🔮 Day Predictor : L'Oracle Temporel

Bienvenue dans le futur de la chronologie assistée par ordinateur.

Avez-vous déjà passé une nuit blanche à vous demander : *"Mon Dieu, mais quel jour serons-nous demain ?"* ?
Avez-vous déjà eu peur de vous réveiller un mardi en pensant que c'était dimanche, provoquant ainsi l'effondrement de votre réalité socio-professionnelle ?
...
- **Satellites de la NASA** (détournés illégalement, chut) pour la synchronisation atomique.
...
1. Clonez ce dépôt (dans une faille spatio-temporelle de préférence).
2. Installez les modules (nécessite une connexion intergalactique) :
   ```bash
   npm install
   ```
3. Lancez le réacteur à fusion froide :
   ```bash
   npm run dev
   ```
...
---
*Développé avec TypeScript, Sass et beaucoup trop de temps libre.*
